...
class PSB:
    ...
    def bm_set_path(self) -> str:
        return os.path.join(self.set_path, PSB.relative_bm_set_path)

    @property
    def classifications_path(self) -> str:
        return os.path.join(self.set_path, PSB.relative_classification_path)
    ...
    def init_psb_fv_variation(self, number_of_points: int, winding_speed: int, p_min:int, c_number: int, filename_index: int) -> 'PSBFVVariantion':
        return PSBFVVariantion(self.set_path, number_of_points, winding_speed, p_min, c_number, filename_index)
